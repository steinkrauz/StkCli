...
namespace StkCli {
	...
	public class ArgHandler<T>where T:new()
	{
		...
		private void SetAttr(PropertyInfo pi, StrParamAttribute attr, string[] args)
		{
			...
			try{
				pi.SetValue(obj, args[Idx+1], new object[0]);
			} catch (Exception sfe) {
				RaiseException(attr, sfe);
			}
		}
		...
		private void SetAttr(PropertyInfo pi, IntParamAttribute attr, string[] args)
		{
			...
			try{
				pi.SetValue(obj, Int32.Parse(args[Idx+1]), new object[0]);
			} catch (Exception sfe) {
				RaiseException(attr, sfe);
			}
		}
		...
		private void SetAttr(PropertyInfo pi, FloatParamAttribute attr, string[] args)
		{
			...
			try {
				pi.SetValue(obj, Double.Parse(args[Idx+1]), new object[0]);
			} catch (Exception sfe) {
				RaiseException(attr, sfe);
			}
		}
		...
		private void SetAttr(PropertyInfo pInfo, AutoHelpAttribute attribute, string[] args)
		{
			...
			foreach (var pi in theType.GetProperties()) {
				...
				foreach (var attr in attrs) {
					if (attr is BaseParamAttribute baseAttr) {
						PrintAttr(baseAttr); 
						break;
					}
				}
		...
		public T Parse(string[] args)
		{
			...
			foreach (var pi in theType.GetProperties()) {
				Attribute[] attrs = Attribute.GetCustomAttributes(pi);
				foreach (var attr in attrs) {
					if (attr is IntParamAttribute intAttr) {
						SetAttr(pi, intAttr, args); 
						break;
					}
					if (attr is StrParamAttribute strAttr)
					{
						SetAttr(pi, strAttr, args);
						break;
					}
					if (attr is FloatParamAttribute floatAttr)
					{
						SetAttr(pi, floatAttr, args);
						break;
					}
					if (attr is FlagParamAttribute flagAttr)
					{
						SetAttr(pi, flagAttr, args);
						break;
					}
					if (attr is AutoHelpAttribute helpAttr)
					{
						SetAttr(pi, helpAttr, args);
						break;
					}
				}
			}
